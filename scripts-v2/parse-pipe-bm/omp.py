...
for device_id, df in results.items():
    ...
    for stage, stage_df in df.groupby("Stage"):
        ...
        print(f"Stage {stage}: Fastest on {fastest_core['Core_Type_Name']} cores ({fastest_core['Time_ms']} ms)")
...
for device_id, df in results.items():
    ...
    # Customize the plot
    plt.title(f'OMP Performance by Stage and Core Type - Device: {device_id}', fontsize=16)
    ...
    # Plot heatmap
    ax = sns.heatmap(heatmap_data, annot=True, fmt='.1f', cmap='YlGnBu_r', linewidths=.5)
    ...
    # Melt for seaborn
    melted_df = pd.melt(speedup_df, id_vars=['Stage'], var_name='Core_Type', value_name='Speedup')
