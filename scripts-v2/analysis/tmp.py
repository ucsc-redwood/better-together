...
def compute_statistics(tasks):
    ...
    stats = {
        ...
        "chunk_std": statistics.stdev(all_chunk_durations) if len(all_chunk_durations) > 1 else 0,
    }
    return stats
...
def main():
    ...
    for task_id, drange, tmin, tmax in sorted(task_ranges, key=lambda x: x[0]):
        print(f"{task_id:7d} | {us_to_ms(drange):12.3f} | {us_to_ms(tmin):15.3f} | {us_to_ms(tmax):15.3f}")
    
    if widest_task:
        wt_id, wt_range, wt_min, wt_max = widest_task
        print("\nWidest Task:")
        print(f"  Task ID: {wt_id}")
        print(f"  Range of chunk durations: {us_to_ms(wt_range):.3f} ms (min: {us_to_ms(wt_min):.3f} ms, max: {us_to_ms(wt_max):.3f} ms)")
