...
def calculate_statistics(tasks):
    ...
    for chunk_idx in range(stats["chunk_count_per_task"]):
        durations = [
            task["chunks"][chunk_idx]["duration"]
            for task in tasks
            if chunk_idx < len(task["chunks"])
        ]
...
def print_statistics(file_name, stats):
    ...
    print("\nChunk Statistics:")
    print(
        f"{'Chunk ID':<10}{'Min':<15}{'Max':<15}{'Mean':<15}{'Median':<15}{'Geo Mean':<15}{'Std Dev':<15}"
    )
    print(f"{'-'*80}")
    ...
    print(
        f"Widest Task: Task {stats['widest_task']['id']} "
...
def compare_schedules(schedules_stats):
    ...
    schedules_by_mean = sorted(
        schedules_stats.items(), key=lambda x: x[1]["task_duration_stats"]["mean"]
    )
    ...
    schedules_by_geo_mean = sorted(
        schedules_stats.items(), key=lambda x: x[1]["task_duration_stats"]["geo_mean"]
    )
    ...
    chunk_count = min(
        stats["chunk_count_per_task"] for stats in schedules_stats.values()
    )

    for chunk_id in range(chunk_count):
        print(f"\nChunk {chunk_id} Mean Duration Comparison:")
        chunk_means = [
            (schedule, stats["chunk_stats"][chunk_id]["mean"])
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze benchmark files for task and chunk statistics."
    )
    parser.add_argument(
        "files",
        metavar="FILE",
