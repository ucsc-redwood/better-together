...
CYCLES_TO_MS = 1.0 / 24576000.0  # Convert cycles to milliseconds
...
for task_id in tasks:
    for chunk_id in tasks[task_id]:
        if "start" in tasks[task_id][chunk_id] and "duration_cycles" in tasks[task_id][chunk_id]:
...
chunk_names = [f"Chunk {i} ({chunk_types.get(i, 'Unknown')})" for i in range(max_chunk_id + 1)]
...
ax.set_title(f"Chunk Execution Timeline ({args.start_time*100:.0f}%-{args.end_time*100:.0f}% of execution)", fontsize=14)
