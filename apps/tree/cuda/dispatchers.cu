...
// ----------------------------------------------------------------------------
// AppData overloads: genuinely chained (single buffer per field), same kernel
// launches as the SafeAppData overloads above -- only the buffer wiring
// changes (each stage now reads the actual previous stage's real output
// instead of the golden-const snapshot). See dispatchers.cuh for why these
// aren't new logic, just re-wiring.
// ----------------------------------------------------------------------------

void CudaDispatcher::run_stage_1_async(tree::AppData& appdata) {
  LOG_KERNEL(LogKernelType::kCUDA, 1, &appdata);
...
void CudaDispatcher::run_stage_2_async(tree::AppData& appdata) {
  LOG_KERNEL(LogKernelType::kCUDA, 2, &appdata);

  const uint32_t* d_keys_in = appdata.u_morton_keys_s1.data();
  uint32_t* d_keys_out = appdata.u_morton_keys_sorted_s2.data();
  uint32_t num_items = appdata.get_n_input();
...
void CudaDispatcher::run_stage_3_async(tree::AppData& appdata) {
  const uint32_t* d_in = appdata.u_morton_keys_sorted_s2.data();
  uint32_t* d_out = appdata.u_morton_keys_unique_s3.data();
  uint32_t num_items = appdata.get_n_input();
...
void CudaDispatcher::run_stage_4_async(tree::AppData& appdata) {
  LOG_KERNEL(LogKernelType::kCUDA, 4, &appdata);
...
void CudaDispatcher::run_stage_5_async(tree::AppData& appdata) {
  LOG_KERNEL(LogKernelType::kCUDA, 5, &appdata);
...
void CudaDispatcher::run_stage_6_async(tree::AppData& appdata) {
  LOG_KERNEL(LogKernelType::kCUDA, 6, &appdata);
  ...
  // -------- host --------------
  appdata.set_n_octree_nodes(appdata.u_edge_offset_s6[appdata.get_n_brt_nodes() - 1]);
  // ----------------------------
...
void CudaDispatcher::run_stage_7_async(tree::AppData& appdata) {
  LOG_KERNEL(LogKernelType::kCUDA, 7, &appdata);
