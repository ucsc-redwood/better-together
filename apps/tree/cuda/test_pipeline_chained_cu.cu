...
// tree::AppData dispatch path on a HYBRID OMP+CUDA schedule -- an OMP CPU
// chunk and a CUDA GPU chunk over the SAME pool, so the CPU writes one item's
// buffers while the GPU reads/writes another's concurrently. NOT part of the
// differential/oracle suite (test-tree-cu) or the concurrency-mechanics suite
// (test-pipeline-e2e-cu), which both keep using tree::SafeAppData unchanged.
//
// What this proves: the existing chunk-boundary cudaDeviceSynchronize() (see
// apps/tree/cuda/dispatchers.cuh's AppData dispatch_multi_stage overload) is
// sufficient for a hybrid schedule's GPU->CPU handoff to see genuinely-chained
// (not golden-const) data correctly -- each pooled item's final octree matches
// a straightforward single-threaded OMP reference pass over the same input.
//
...
// through the extracted OMP stage functions sequentially (the oracle -- no
// pool/ring/GPU), then diff every terminal buffer against what the hybrid
// pool/ring (OMP stages + CUDA stages) produced.
...
// Hybrid CPU+GPU: OMP stages 1-3 (CPU), CUDA stages 4-7 (GPU), over the SAME
// genuinely-chained AppData pool -- the concurrent visibility path §1 tested
// with real chaining instead of golden-const inputs.
TEST(PipelineChainedCu, HybridOmpCuda) {
  if (!CudaAvailable()) GTEST_SKIP() << "no CUDA device";
  Schedule sched;
  sched.uid = "chained-omp-cu";
  sched.chunks = {
  ...
  bt_pipe_test::run_pipeline<tree::AppData, tree::cuda::CudaDispatcher, QueueT>(
