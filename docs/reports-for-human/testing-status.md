...
> cross-backend correctness suite. Companion to
> [`cmake-migration-rfc.md`](cmake-migration-rfc.md) (the build).
> For the commands to actually run/write tests, see
> [`../instruction-for-ai/03-unit-testing.md`](../instruction-for-ai/03-unit-testing.md).
...
[`../instruction-for-ai/04-alexnet-cifar-spec.md`](../instruction-for-ai/04-alexnet-cifar-spec.md)
makes **`AlexNetCIFAR` (11 stages)** canonical, but the C++ kernels still implement
the old **`SmallAlexNet` (9 stages, 1024→10 single FC)** — the two are **not**
shape/weight-compatible. Adopting the canonical model re-shapes `AppData` and every
cifar `run_stage_*`.
