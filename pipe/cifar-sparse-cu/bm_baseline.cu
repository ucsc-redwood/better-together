...
static void BM_baseline_omp_little(benchmark::State& state) {
  ...
  // Benchmark
  for (auto _ : state) {
    cifar_sparse::omp::dispatch_multi_stage(LITTLE_CORES, appdata, 1, 9);
  }
}
...
static void BM_baseline_omp_medium(benchmark::State& state) {
  ...
  // Benchmark
  for (auto _ : state) {
    cifar_sparse::omp::dispatch_multi_stage(MEDIUM_CORES, appdata, 1, 9);
  }
}
...
static void BM_baseline_omp_big(benchmark::State& state) {
  ...
  // Benchmark
  for (auto _ : state) {
    cifar_sparse::omp::dispatch_multi_stage(BIG_CORES, appdata, 1, 9);
  }
}
