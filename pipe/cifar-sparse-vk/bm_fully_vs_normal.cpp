...
static void BM_run_fully(BmTable<kNumStages>& table,
  ...
  // a CPU thread and the GPU thread mutated the same buffers concurrently -- a
  // data race. cifar's fixed-shape convs tolerate it (unlike tree, which hangs),
  // but it still contaminates the btpm timings, so remove it here too.
  ...
  if (has_lit_cores()) {
    threads.emplace_back([&]() {
      while (!done.load(std::memory_order_relaxed)) {
        ...
        cifar_sparse::omp::dispatch_multi_stage(LITTLE_CORES, *app, stage, stage);
        lit_processed++;
  ...
  if (has_med_cores()) {
    threads.emplace_back([&]() {
      while (!done.load(std::memory_order_relaxed)) {
        ...
        cifar_sparse::omp::dispatch_multi_stage(MEDIUM_CORES, *app, stage, stage);
        med_processed++;
  ...
  if (has_big_cores()) {
    threads.emplace_back([&]() {
      while (!done.load(std::memory_order_relaxed)) {
        ...
        cifar_sparse::omp::dispatch_multi_stage(BIG_CORES, *app, stage, stage);
        big_processed++;
...
//
// Run with (on Android)
// xmake r bm-fully-cifar-sparse-vk --device 3A021JEHN02756 -l off -p -t 10
//
// ----------------------------------------------------------------------------
