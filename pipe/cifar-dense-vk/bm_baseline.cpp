// Single-PU "no framework" baselines for cifar-dense x Vulkan. All registration +
...
#include "../bm_baseline_common.hpp"
#include "const.hpp"

int main(int argc, char** argv) {
  return bt_baseline::run<AppDataT, DispatcherT>(
      argc, argv, static_cast<int>(kNumStages), "CIFAR-Dense", "VK",
      [](AppDataT& a, int lo, int hi) { cifar_dense::omp::dispatch_multi_stage(a, lo, hi); },
      [](std::vector<int>& c, size_t n, AppDataT& a, int lo, int hi) {
        cifar_dense::omp::dispatch_multi_stage(c, n, a, lo, hi);
      },
