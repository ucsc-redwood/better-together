...
- **Where each backend runs:** OMP runs **anywhere** and is the **reference oracle**.
  ...
  comparable. CUDA 13's CUB removal breaks the build (PC **and** native Jetson), so the
  cross container stays on CUDA 12.6 — its binaries are smoke-verified to run on
  JetPack 7.2 (2026-07-02). Vulkan needs an **integrated GPU** — `kiss-vk` hard-selects
