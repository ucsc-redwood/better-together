...
#include "h/cifar_dense_conv2d_spv.h"
#include "h/cifar_dense_linear_spv.h"
#include "h/cifar_dense_maxpool_spv.h"
...
#define SHADER_ENTRY(name)                                                                   \
  {                                                                                          \
    #name, {                                                                                 \
      play_ndarray_vk_shaders_spv_##name##_spv, play_ndarray_vk_shaders_spv_##name##_spv_len \
    }                                                                                        \
  }
...
static const std::unordered_map<std::string, std::pair<const unsigned char*, size_t>> all_shaders =
    {
        SHADER_ENTRY(cifar_dense_conv2d),
        SHADER_ENTRY(cifar_dense_linear),
        SHADER_ENTRY(cifar_dense_maxpool),
