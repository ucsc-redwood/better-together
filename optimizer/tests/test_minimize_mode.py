...
HUGE = 1000.0
# [Little, Medium, Big, GPU] per stage (app="cifar-dense" -> 11 stages). Little/Medium are
# huge (never picked); Big is cheap except stage 0; GPU is cheap except stage 10. So any
# single-PU schedule costs ~110 ms (gap 0), while the makespan-optimal SPLIT (GPU 0..9,
# Big 10) costs ~10 ms (gap ~9). The degenerate old gapness loved the 110 ms gap-0 answer.
STAGES = [[HUGE, HUGE, (100.0 if s == 0 else 1.0), (100.0 if s == 10 else 1.0)] for s in range(11)]


def _primary_makespan(mode):
    # solutions[0] is the optimizer's OPTIMUM for the chosen objective.
    sols = solve_optimization_problem(
        STAGES, num_solutions=20, app_name="cifar-dense", minimize_mode=mode
    )
...
def test_absent_tier_never_assigned():
    ...
    stages = [[UNAVAILABLE, 2.0, 3.0, 1.0] for _ in range(11)]
    for mode in ("gapness", "max_time"):
        sols = solve_optimization_problem(
            stages, num_solutions=10, app_name="cifar-dense", minimize_mode=mode
        )
        for sol in sols:
