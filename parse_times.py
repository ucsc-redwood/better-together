...
PREDICTED_TIMES = [
    8.6717, 8.6717, 9.07, 9.1116, 9.1116, 9.1116, 9.1116, 9.2635, 9.2635, 9.5876,
    9.6043, 9.6043, 9.6183, 9.6358, 9.9833, 9.9833, 9.9833, 9.9833, 9.9833, 9.9833,
    9.9833, 9.9833, 9.9833, 9.9833, 10.1, 10.1, 10.3447, 10.3447, 10.3447, 10.3447
]
...
def create_visualization(avg_executions, geo_executions, std_executions, predicted_times):
    ...
    plt.errorbar(x, avg_executions, yerr=std_executions, 
                fmt='o-', label='Measured (Arithmetic)', capsize=5, 
...
def main():
    ...
    create_visualization(avg_executions, geo_executions, std_executions, predicted_times)
    ...
    print(f"Arithmetic mean correlation: {correlation_arith:.4f} (p-value: {p_value_arith:.4f})")
    print(f"Geometric mean correlation: {correlation_geo:.4f} (p-value: {p_value_geo:.4f})")
    ...
    print("=" * 120)
    print(f"{'Execution':<12} {'Arith (s)':<12} {'Geo (s)':<12} {'Predicted (s)':<12} {'Diff Arith %':<12} {'Diff Geo %':<12} {'Error %':<12}")
    print("-" * 120)
    
    for i, (avg, geo, pred, diff_arith_pct, diff_geo_pct, err_pct) in enumerate(
        zip(avg_executions, geo_executions, predicted_times, diff_percent_arith, diff_percent_geo, percent_error), 1):
        print(f"{i:<12} {avg:.4f} {geo:.4f} {pred:.4f} {diff_arith_pct:+.1f}% {diff_geo_pct:+.1f}% {err_pct:.1f}%")
