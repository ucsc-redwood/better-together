...
def extract_solution(model, x, num_stages, core_types, stage_timings):
            ...
            chunks.append(
                {
                    ...
                    "stages": stages.copy(),
                    "time": sum(
                        stage_timings[s][core_types.index(curr)] for s in stages
                    ),
                }
            )
            curr = assigns[i]
    ...
    uid = (
        f"SCH-{summary}-G{gap_str}-{hashlib.md5(str(chunks).encode()).hexdigest()[:4]}"
    )
    return {
        "uid": uid,
        "chunks": chunks,
...
def solve(stage_timings, num_solutions):
    ...
    T_max, T_min, Gapness = add_timing_opt(
        opt, x, num_stages, core_types, stage_timings
    )
...
def main():
    ...
    parser.add_argument(
        "--csv_folder",
        required=True,
        help="Folder with device_app_backend_{normal,fully}.csv",
    )
    ...
    parser.add_argument(
        "--output_folder", required=True, help="Root output for JSON schedules"
    )
    ...
    for kind in ["normal", "fully"]:
        ...
        out_path = os.path.join(
            args.output_folder, f"{device}_{app}_{backend}_{kind}_schedules.json"
        )
