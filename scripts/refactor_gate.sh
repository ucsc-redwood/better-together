...
build_and_run() {
  ...
  if ! cmake --build --preset "$RUN_PRESET" >"$out/build.log" 2>&1; then
    tag "BUILD FAILED -- see $out/build.log"
...
cmd_capture() {
  rm -rf "$STORE"
  build_and_run "$STORE" || {
    echo "baseline capture aborted (build failed)"
    exit 1
  }
  echo
...
cmd_check() {
  [ -d "$STORE" ] || {
    echo "no baseline -- run 'capture-baseline' first"
    exit 2
  }
  local cur
  cur="$(mktemp -d)"
  ...
  echo
  echo "=== gate: equivalence vs baseline ==="
  ...
  for base in "$STORE"/inventory-*.txt; do
    ...
    if [ ! -f "$now" ]; then
      echo "  [$p] MISSING now (build dir gone?)  <-- FAIL"
      rc=1
      continue
    fi
    local nb nn
    ...
    nn=$(wc -l <"$now")
    if [ "$nn" -eq 0 ]; then
      echo "  [$p] 0 tests now ('green' with no tests is a lie)  <-- FAIL"
      rc=1
    fi
  ...
  echo
  if [ "$rc" -eq 0 ]; then
    echo "GATE GREEN -- behavior preserved, safe to commit this phase."
