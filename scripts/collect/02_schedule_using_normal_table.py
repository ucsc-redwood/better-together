...
def add_baseline_constraint(opt, T_max, baseline_time):
    ...
    print(
        f"Adding constraint: Schedule must be faster than baseline ({baseline_time} ms)"
    )
...
def solve_optimization_problem(
    ...
    # Initialize data
    num_stages, core_types, normal_stage_timings = define_data(stage_timings, app_name)
    ...
    T_max, T_min, Gapness = add_chunk_time_constraint(
        opt, x, core_types, num_stages, normal_stage_timings
    )
    ...
    while solution_count < num_solutions and opt.check() == sat:
        ...
        # Print details
        # print_stage_assignments(m, x, num_stages, core_types, normal_stage_timings)
        print_stage_assignments_v2(m, x, num_stages, core_types, normal_stage_timings)
        chunk_times = print_chunk_summary(
            m, x, num_stages, core_types, normal_stage_timings
        )
...
if __name__ == "__main__":
    ...
    if args.csv_folder:
        ...
        # =========================================================================================================
        csv_path = os.path.join(args.csv_folder, device, app, backend, "normal.csv")
        # =========================================================================================================
        ...
        if args.output_folder:
            ...
            out_path = os.path.join(
                args.csv_folder, device, app, backend, "schedules_normal.json"
            )
