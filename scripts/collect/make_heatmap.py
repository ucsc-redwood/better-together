...
def plot_bar_chart(mean_normal, std_normal, mean_fully, std_fully,
                   device, out_folder, app, backend_col):
    ...
    for ax, (mean_df, std_df), title in zip(axs, data_pairs, titles):
        for i, pu in enumerate(pu_list):
            ...
            ax.bar(pos, mean_df[pu], width,
                   yerr=std_df[pu], capsize=5, label=pu)
        ax.set_xticks(x)
...
def main():
    ...
    parser.add_argument('--log_folder', required=True,
                        help='Folder with CSVs')
    parser.add_argument('--app', required=True,
                        help='Application name (e.g., cifar-sparse)')
    parser.add_argument('--backend', required=True, choices=BACKEND_COL_MAP.keys(),
                        help='Backend: vk or cu')
    parser.add_argument('--device', required=True,
                        help='Device identifier')
    parser.add_argument('--exclude_stages', default='',
                        help='Comma-separated stages to exclude')
    ...
    if exclude:
        ...
        mean_n, std_n, mean_f, std_f = (
            mean_n[mask], std_n[mask], mean_f[mask], std_f[mask]
        )
        cv_n = cv_n[mask]
        cv_f = cv_f[mask]
    ...
    avg_table = pd.concat([
        mean_n.add_prefix('normal_'),
        mean_f.add_prefix('fully_')], axis=1)
    print('\nAverage Table (ms):')
    print(avg_table.round(3))

    plot_bar_chart(mean_n, std_n, mean_f, std_f,
                   device, folder, app, backend_col)
    diff = mean_f - mean_n
