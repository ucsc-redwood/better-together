...
def main():
    ...
    parser.add_argument("device", help="Device ID (e.g., 3A021JEHN02756)")
    parser.add_argument("app", help="Application name (e.g., tree)")
    parser.add_argument("backend", help="Backend name (e.g., vk, cu)")
    ...
    for i in range(1, 6):
        ...
        sched_path = (
            f"data/schedules/{device}/{app}/{backend}/" "schedules_isolated_tmax.json"
        )
        ...
        cmd = [
            "uv",
            "run",
            "scripts/collect/04_parse_schedules.py",
            "-v",
            log_path,
            "--max-schedules",
            "30",
        ...
        for line in proc.stdout.splitlines():
            if "Pearson correlation coefficient" in line:
                ...
                try:
                    ...
                    print(
                        f"[warn] couldn't parse value in line: {line}", file=sys.stderr
                    )
                break
