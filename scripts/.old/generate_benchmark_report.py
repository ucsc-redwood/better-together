...
def read_and_aggregate_data():
    ...
    for file_path in csv_files:
        device, app, backend, run_type = get_device_app_backend_type_from_filename(
            file_path
        )
    ...
    for run_type in ["fully", "normal"]:
        ...
        for i, device in enumerate(devices):
            for j, app in enumerate(applications):
                ...
                subset = run_type_data[
                    (run_type_data["device"] == device) & (run_type_data["app"] == app)
                ]
                ...
                for stage in subset["stage"].unique():
                    ...
                    ax.bar(
                        range(len(processors)), proc_values, label=f"Stage {int(stage)}"
                    )
...
def create_heatmap(data):
    ...
    for run_type in ["fully", "normal"]:
        ...
        for i, device in enumerate(devices[:2]):  # Limit to 2 devices for the 2x3 grid
            for j, app in enumerate(
                applications[:3]
            ):  # Limit to 3 apps for the 2x3 grid
                subset = run_type_data[
                    (run_type_data["device"] == device) & (run_type_data["app"] == app)
                ]
                ...
                sns.heatmap(
                    ...
                    fmt=".2f",
                    cmap="RdBu_r",
                    xticklabels=processors,
                    yticklabels=processors,
                    ax=ax,
                    vmin=-4,
                    vmax=4,
                    center=0,
        ...
        for device in devices:
            for app in applications:
                ...
                for i, processor in enumerate(processors):
                    values = [
                        subset[subset["stage"] == stage][processor].values[0]
                        for stage in stages
                    ]
                ...
                ax.set_ylabel("Time (ms) - Log Scale")
                ax.set_title(
                    f"Stage Execution Times for {app} on {device} ({run_type})"
                )
                ...
                plt.tight_layout()
                plt.savefig(
                    os.path.join(FIG_DIR, f"stages_{device}_{app}_{run_type}.png")
                )
                plt.close()
...
def create_device_comparison_plots(data):
    ...
    for run_type in ["fully", "normal"]:
        ...
        for app in applications:
            ...
            plt.tight_layout()
            plt.savefig(
                os.path.join(FIG_DIR, f"device_comparison_{app}_{run_type}.png")
            )
            plt.close()
...
def create_summary_table(data):
    ...
    pivot = summary.pivot_table(
        index=["app", "run_type"], columns=["device", "processor"], values="mean_time"
    )
